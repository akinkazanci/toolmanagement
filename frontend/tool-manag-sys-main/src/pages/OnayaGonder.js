...
const OnayaGonder = () => {
  ...
  const postNotify = async (payload) => {
    try {
      const resLocal = await fetch(`${API_BASE_URL}/UserApproval/notify-admin`, {
        method: 'POST',
  ...
  return (
    <div className="min-h-screen p-6">
      ...
      <div className="bg-white/80 backdrop-blur-sm rounded-xl border border-slate-200 shadow-lg p-4 mb-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Ara..."
              className="pl-9 pr-3 py-2 bg-white border border-slate-300 rounded-lg w-full text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-blue-400"
              value={searchTerm}
          ...
          <select
            ...
            className="px-3 py-2 bg-white border border-slate-300 rounded-lg w-full text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-blue-400"
          >
        ...
        <div className="mt-4 flex justify-end">
          <button
            ...
            className="px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex items-center gap-2 shadow-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-blue-400"
          >
      ...
      {loading ? (
        <div className="p-6 text-slate-600">Yükleniyor...</div>
      ) : (
        <div className="bg-white/80 backdrop-blur-sm rounded-xl border border-slate-200 shadow-lg overflow-x-auto">
          <table className="min-w-full">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                <th className="text-left text-xs font-semibold text-slate-600 uppercase tracking-wider px-4 py-3">Kullanıcı</th>
                <th className="text-left text-xs font-semibold text-slate-600 uppercase tracking-wider px-4 py-3">Departman</th>
                <th className="text-left text-xs font-semibold text-slate-600 uppercase tracking-wider px-4 py-3">Lokasyon</th>
                <th className="text-left text-xs font-semibold text-slate-600 uppercase tracking-wider px-4 py-3">Durum</th>
                <th className="text-left text-xs font-semibold text-slate-600 uppercase tracking-wider px-4 py-3">Uygulama</th>
                <th className="text-left text-xs font-semibold text-slate-600 uppercase tracking-wider px-4 py-3">Erişim Yetkisi</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-200">
              {filteredRows.map((row, idx) => (
                <tr key={`${row.userId}-${row.appId}-${idx}`} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 text-sm text-slate-600">{(row.status || '').toLowerCase() || '-'}</td>
                  <td className="px-4 py-3 text-sm text-slate-600">{row.appName || '-'}</td>
                  <td className="px-4 py-3 text-sm text-slate-600">{row.roles && row.roles.length > 0 ? row.roles.join(', ') : '-'}</td>
                </tr>
              ))}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 p-4 flex items-center justify-center">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl overflow-hidden border border-slate-200 relative">
            <div className="flex items-center justify-between p-6 border-b border-slate-200">
              <h2 className="text-lg font-semibold text-slate-900">Onaya Gönder</h2>
              <button onClick={() => setShowSendModal(false)} className="w-8 h-8 bg-slate-100 hover:bg-slate-200 border border-slate-200 rounded-lg flex items-center justify-center">
                <X className="h-4 w-4 text-slate-600" />
              </button>
            </div>
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-2">Yönetici E-posta</label>
                <input
                  type="email"
                  value={adminEmail}
                  onChange={(e) => setAdminEmail(e.target.value)}
                  placeholder="ornek@firma.com"
                  className="w-full px-3 py-2 bg-white border border-slate-300 rounded-lg text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-blue-400"
                />
              </div>
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-semibold text-slate-900">Pending Kullanıcılar</h3>
                <button type="button" onClick={toggleSelectAll} className="text-sm text-blue-600 hover:text-blue-700">Tümünü Seç</button>
              </div>
