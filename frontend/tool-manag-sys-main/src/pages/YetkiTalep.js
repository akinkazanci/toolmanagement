...
import { API_BASE_URL } from '../config'; // ✅ EKLE
...
export default function YetkiTalep() {
  ...
  // ✅ Kullanıcıları çek
  useEffect(() => {
    fetch(`${API_BASE_URL}/Users`) // ✅ DEĞİŞTİ
      .then((res) => res.json())
      .then(setUsers)
  ...
  // ✅ Uygulamaları çek
  useEffect(() => {
    fetch(`${API_BASE_URL}/Applications`) // ✅ DEĞİŞTİ
      .then((res) => res.json())
      .then(setApps)
  ...
  useEffect(() => {
    if (appId) {
      fetch(`${API_BASE_URL}/Projects/by-app/${appId}`) // ✅ DEĞİŞTİ
        .then((res) => res.json())
        .then(setProjects)
        .catch((err) => console.error("Projeler alınamadı:", err));

      fetch(`${API_BASE_URL}/Permission/by-app/${appId}`) // ✅ DEĞİŞTİ
        .then((res) => res.json())
        .then(setPermissions)
  ...
  const handleSave = () => {
    ...
    fetch(`${API_BASE_URL}/UserProjectPermissions`, { // ✅ DEĞİŞTİ
      method: "POST",
  ...
  return (
    <div className="p-6 min-h-screen">
      <div className="max-w-3xl mx-auto bg-white p-6 rounded-lg shadow">
        <h1 className="text-4xl font-bold text-slate-900 mb-6 flex items-center">
