...
export default function ProjectManagement() {
  ...
  return (
    <div className="min-h-screen relative overflow-hidden">
      ...
      {/* Onaya Gönder Kutusu */}
      <div className="mx-6 my-8 bg-white rounded-xl border border-slate-200 shadow-lg p-6">
        <h2 className="text-xl font-semibold mb-4 text-slate-900">{t("send_to_project_manager") || "Proje yöneticisine onaya gönder"}</h2>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {/* Kullanıcı */}
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">{t("select_user")}</label>
            <select
              value={selectedUser}
          ...
          {/* Uygulama */}
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">{t("select_application")}</label>
            <select
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
          <div className="md:col-span-2">
            <label className="block text-sm font-medium text-slate-700 mb-2">{t("project_manager_email") || "Proje Yöneticisi E-posta"}</label>
            <input
              type="email"
              value={pmEmail}
        ...
        <div className="mt-6">
          <div className="bg-white border border-slate-200 rounded-lg p-4">
            <h3 className="text-lg font-semibold mb-2 text-slate-900">{t("send_approval_title")}</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
              <div>
                <span className="text-slate-500">{t("user_column")}: </span>
                <span className="font-semibold text-slate-900">{selectedUserObj?.fullName || selectedUserObj?.username || t("not_specified")}</span>
              </div>
              <div>
                <span className="text-slate-500">{t("application")}: </span>
                <span className="font-semibold text-slate-900">{selectedApp?.appName || t("not_specified")}</span>
              </div>
              <div>
                <span className="text-slate-500">{t("projects")}: </span>
